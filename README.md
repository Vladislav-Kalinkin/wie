...
| C++ EH                         | MSVC `_CxxThrowException` enters two-pass SEH (host FuncInfo). Catch is reached (7za can print e.g. “Can't allocate…” on `-md=64k`) but full catch ABI / UnwindMap dtors / RTTI still incomplete → may fault after the message. Mingw LSDA for micros still partial. |
